...
impl ScalarIndex for BitmapIndex {
    ...
    async fn update(
        ...
        let page_lookup_file = self.lazy_reader.get().await?;
        let schema = Schema::from(page_lookup_file.schema());
        let old_data_stream = try_stream! {
            let total_rows = page_lookup_file.num_rows();
            for start_row in (0..total_rows).step_by(MAX_ROWS_PER_CHUNK) {
                let end_row = (start_row + MAX_ROWS_PER_CHUNK).min(total_rows);
                let chunk = page_lookup_file
...
impl BitmapIndexPlugin {
    ...
    async fn do_train_bitmap_index(
        ...
        let merged_stream = try_stream! {
            let mut left_items: Pin<Box<dyn Stream<Item = Result<(ScalarValue, RowIdTreeMap)>> + Send>> = Box::pin(try_stream! {
            ...
            let mut right_items: Pin<Box<dyn Stream<Item = Result<(ScalarValue, RowIdTreeMap)>> + Send>> = Box::pin(try_stream! {
...
pub mod tests {
    ...
    async fn test_big_bitmap_index() {
        ...
        // read by chunks
        let result =
            BitmapIndexPlugin::write_bitmap_index(Box::pin(data), &test_store, &DataType::UInt32).await;
